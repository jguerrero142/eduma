	...
	* */
	...
	var thim_eduma = {
			...
			* */
			...
			$(document).on('click', '#lp-archive-courses > .learn-press-pagination a.page-numbers', function (e) {
				...
				if (body.hasClass('archive') && body.hasClass('category') &&
					...
					var regexCheckCourseCategoryId = new RegExp('category-([0-9]+)');
		...
		full_right: function () {
			...
			// $('.dot_line_buttom_top').each(function () {
			// 	var width_div = jQuery(this).find('.smicon-box .content-inner').width() / 2;
			// 	jQuery(this).find('.smicon-box .line_top').css('margin-left', width_div + 'px');
			// });
		},
