...
class LP_PMS_Order {
	...
	public function __construct() {
		...
		LP_PMS_Order::$_LIMIT_COURSES              = apply_filters( 'learn-press/pmspro/limit-courses', 10 );
		LP_PMS_Order::$_LIMIT_ORDERS               = apply_filters( 'learn-press/pmspro/limit-orders', 10 );
		LP_PMS_Order::$_CALL_CRON_JOB_AFTER_SECOND = apply_filters( 'learn-press/pmspro/time-call-cron-job-after-second', 5 );
	...
	public function create_lp_order( $user_id = 0, $level_id = 0 ) {
		...
		switch ( $run_mode ) {
			case 'background':
				...
				wp_schedule_single_event( time() + LP_PMS_Order::$_CALL_CRON_JOB_AFTER_SECOND, 'hook_single_event_add_courses_to_lp_order', $paramsCronjob );
	...
	public function handleAddItemToLpOrderCurl( $params ) {
		...
		LP_PMS_Handle_Curl::curlMultipleExec( $curlMultiProcess, $curlArr, function ( $response ) use ( &$total_item_success ) {
	...
	public function LpOrderCancel( $user_id, $cancel_level_id ) {
		...
		if ( $lpOrder && isset( $lpOrder->post_id ) ) {
			...
			$resultStatusLpOrderItems = LP_PMS_DB::getInstance()->updateStatusCoursesByOrderAndUser( $user_id, $lpOrder->post_id, 'cancelled' );
	...
	public function addCoursesToOrders( $lp_orders, $new_course_ids = array() ) {
		...
		switch ( $run_mode ) {
			case 'background':
				$totalPageOrder = ceil( $total_orders / LP_PMS_Order::$_LIMIT_ORDERS );
				...
				$paramsCronjob = array(
					'params' => array(
						...
						'p_order'          => 0,
						'limit'            => LP_PMS_Order::$_LIMIT_ORDERS,
					)
				);

				wp_schedule_single_event( time() + LP_PMS_Order::$_CALL_CRON_JOB_AFTER_SECOND, 'hook_single_event_add_courses_to_lp_orders_when_save_level', $paramsCronjob );
	...
	public function handleAddItemsToLpOrdersCronjobWhenCoursesLevelChange( $params = array() ) {
		...
		$end              = $params['p_order'] * LP_PMS_Order::$_LIMIT_ORDERS + LP_PMS_Order::$_LIMIT_ORDERS;
		$i                = $params['p_order'];
	...
	/*public function handleAddItemsToLpOrderCronjobWhenCoursesLevelChange( $params = array() ) {
		...
		if ( isset( $params_rs['code'] ) && $params_rs['code'] == 1 ) {
			...
			if ( $params_rs['p_course'] < $params_rs['total_page_course'] ) {
				wp_schedule_single_event( time() + LP_PMS_Order::$_CALL_CRON_JOB_AFTER_SECOND, 'hook_single_event_add_courses_to_lp_order_when_save_level', $paramsCronjob );
			} else {
				++ $params_rs['p_order'];
	...
	public function update_courses_on_lp_orders_when_save_level( $lp_orders = array(), $level_course_ids = array(), $level_id = 0 ) {
		foreach ( $lp_orders as $lp_order ) {
			...
			if ( count( $remove_course_ids ) > 0 ) {
				LP_PMS_Order::getInstance()->deleteCoursesOnOrder( $lp_order->order_id, $remove_course_ids );
			}
			...
			if ( count( $add_course_ids ) > 0 ) {
				...
				$param_results = LP_PMS_Order::getInstance()->addItemsToLpOrder( $params, $add_course_ids );
			}
		}
	}
