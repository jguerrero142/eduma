...
add_filter( 'learn-press/can-view-item', 'set_co_instructor_can_view_course_assign', 10, 4 ); // For LP3
add_filter( 'learnpress/course/can-view-content', 'set_co_instructor_can_view_course_assign', 10, 4 ); // For LP4
function set_co_instructor_can_view_course_assign( $can_view_item, $item_id, $user_id, $course_id ) {
	$list_coInstructor = get_post_meta( $course_id, '_lp_co_teacher', array() );

	if ( $can_view_item instanceof LP_Model_User_Can_View_Course_Item ) {
		if ( ! $can_view_item->flag && in_array( $user_id, $list_coInstructor ) ) {
			$can_view_item->flag = true;
			$can_view_item->key  = 'co-instructor';
		}
	} elseif ( ! $can_view_item && in_array( $user_id, $list_coInstructor ) ) {
		$can_view_item = 'co-instructor';
	}

	// Check is item created by co-instructor
	/*$post = get_post( $item_id );

	if ( $post ) {
		if ( $post->post_type == LP_LESSON_CPT && $post->post_author == get_current_user_id() ) {
			$can_view_item = 'co-instructor';
		}
	}*/
