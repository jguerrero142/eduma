...
if ( ! class_exists( 'LP_Addon_Co_Instructor' ) ) {
	...
	class LP_Addon_Co_Instructor extends LP_Addon {
		...
		public function __construct() {
			...
			// add_action( 'learn-press/register-emails', array( $this, 'add_co_instructor_emails' ), 11 );
		...
		public function get_items_of_co_instructor( $query ) {
			...
			if ( is_admin() && function_exists( 'get_current_screen' ) &&
			     in_array( LP_TEACHER_ROLE, $current_user->roles ) ) {
				...
				if ( $current_screen && in_array( $current_screen->id, $screen_check_arr ) ) {
					...
					if ( count( $courses ) > 0 ) {
						unset( $query->query_vars['author'] ); // Remove query post by author_id
					}
		...
		public function restrict_co_instructor_items( $views ) {
			...
			foreach ( $new_views as $view => $name ) {
				...
				if ( $result->found_posts > 0 ) {
					$views[ $view ] = sprintf(
						'<a href="%s"' . $class . '>' . __( $name,
							'learnpress-co-instructor' ) . ' <span class="count">(%d)</span></a>',
		...
		public function get_available_questions( $quizzes ) {
			...
			$query     = $wpdb->prepare( "
				...
				WHERE  post_type = %s
				AND post_author = %d",
				'lp_question', get_current_user_id() );
		...
		public function add_meta_box( $meta_box ) {
			...
			$meta_box['fields'][] = array(
				...
				'id'          => "_lp_co_teacher",
				'desc'        => sizeof( $users ) ? __( 'Colleagues will work with you.',
					'learnpress-co-instructor' ) : wp_kses( __( 'There is no instructor to select. Create <a href="' . admin_url( "user-new.php" ) . '" target="_blank">here</a>.',
		...
		public function co_instructor_settings( $settings, $object ) {
			...
			$instructor_setting = array(
				...
				'type'        => 'text',
				'placeholder' => '',
				'desc'        => __( 'This is a slug and should be unique.',
						'learnpress-co-instructor' ) . sprintf( ' %s <code>[profile/admin/instructor]</code>',
						__( 'Example link is', 'learnpress-co-instructor' ) )
			);
			$instructor_setting = apply_filters( 'learn_press_page_settings_item_instructor', $instructor_setting,
				$settings, $object );
		...
		public function single_course_instructors() {
			...
			learn_press_get_template( 'single-course-tab.php', array( 'instructors' => $instructors ),
				learn_press_template_path() . '/addons/co-instructors/', LP_ADDON_CO_INSTRUCTOR_TEMPLATE );
		}
