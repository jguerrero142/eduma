...
class Thim_Add_Meta_Box {
				...
				switch ( $field['type'] ) {
					...
					case 'image_advanced':
						if ( isset( $field['max_file_uploads'] ) && $field['max_file_uploads'] > 1 ) {
							...
							$values     = get_post_meta( $post_id, $field['id'], false ) ?? array();
					...
					case 'group':
						if ( isset( $field['fields'] ) ) {
							$groups = isset( $_POST[ $field['id'] ] ) ? wp_unslash( $_POST[ $field['id'] ] ) : '';

							update_post_meta( $post_id, $field['id'], $groups );
						}
						break;
	...
	public function display( $field ) {
		...
		switch ( $field['type'] ) {
			...
			case 'group':
				if ( isset( $field['fields'] ) ) {
					$values = get_post_meta( $thepostid, $field['id'], true );
					?>

					<div class="form-field thimpress-meta-box-group" data-hide="<?php echo esc_attr( ! empty( $field['hidden'] ) ? htmlentities( wp_json_encode( $field['hidden'] ) ) : '' ); ?>">
						<?php
						foreach ( $field['fields'] as $g_fields ) {
							$g_fields['value'] = isset( $values[$g_fields['id']]) ? $values[$g_fields['id']] : '';

							$g_fields['id'] = $field['id'].'['.$g_fields['id'].']';

							if ( isset( $g_fields['hidden'] ) ) {
								$g_fields['hidden'][0] = $field['id'].'['.$g_fields['hidden'][0].']';
							}
