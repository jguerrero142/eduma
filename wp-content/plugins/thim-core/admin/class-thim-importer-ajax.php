...
class Thim_Importer_AJAX {
	...
	private function _get_dir_current_demo() {
		...
		return apply_filters( 'thim_core_importer_directory_current_demo', $base_dir . $current_demo['demo'],
			$current_demo['demo'] );
	}
		...
		if ( $plugin ) {
			...
			if ( 'not_installed' === $status ) {
				...
				if ( $install ) {
					...
					thim_add_log( sprintf( __( 'Installing %s plugin failed. %s', 'thim-core' ),
						$plugin->get_name(),
						$string ), 'plugin_installation' );
